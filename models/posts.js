...
const dataPattern = /^(\d{4})(\/|-)(\d{1,2})(\/|-)(\d{1,2})$/;
...
const addPostSchema = Joi.object({
  ...
  start: Joi.string().pattern(timePattern).min(5).max(5).required().messages({
  ...
  end: Joi.string().pattern(timePattern).min(5).max(5).required().messages({
  ...
  date: Joi.string().pattern(dataPattern).min(10).max(10).required().messages({
    ...
    start: {
      ...
      validate: {
        validator: function (value) {
          const [hoursStart, minutesStart] = value.split(":");
          const [hoursEnd, minutesEnd] = this.end.split(":");
...
const Task = model("post", postSchema);
